...
if args.error:    
    ...
    all_last_vals = comm.gather(((it_table.last_itr, ry, rx), region.slices[nt-1]), root=0)
    ...
    if rank==0:
        ...
        max_itr = np.max([z[0] for (z, _) in all_last_vals])
        errors = [np.max(np.abs(exact_views[z[1]][z[2]]-v0))
